...
8. [苦労したところ](#苦労したところ)
9. [追加したい機能](#追加したい機能)
10. [ユーザー新規作成の流れ](#ユーザー新規作成の流れ)
11. [出欠席の管理に関して](#出欠席の管理に関して)
...
JWTの保存先を変えたい・・現在はJWTをローカルストレージに保存しているため、JavaScriptからアクセスすることが出来、XSS攻撃される可能性がある。そこで、JWTをcookieに保存して、http onlyをtrueにして送受信するように、セキュリティに注意した開発を行いたい。<br>
xss攻撃とは・・悪意あるスクリプトをwebページに入れてユーザーのブラウザ上で勝手に実行させる攻撃<br>


### ユーザー新規作成の流れ
①カードキー番号をカードリーダーにかざし読み取る。<br>
